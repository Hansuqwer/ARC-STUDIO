...
def test_native_mode_no_gating_required(layout, monkeypatch):
    """Native SwarmGraphRunner runs without cost gating (fake_offline default)."""
    from agent_runtime_cockpit.swarmgraph import SwarmGraphRunner
    from agent_runtime_cockpit.swarmgraph.config import SwarmGraphConfig

    cfg = SwarmGraphConfig(execution_mode="fake_offline", allow_paid_calls=False)
    runner = SwarmGraphRunner(config=cfg)
    result = runner.run("test native gating")
    assert result["status"] == "completed"

def test_cli_path_still_gates(layout, monkeypatch):
    """CLI subprocess path still enforces require_dual_gate."""
    import asyncio
    ...
    # With costs approved, gating passes (subprocess may still fail — not our concern)
