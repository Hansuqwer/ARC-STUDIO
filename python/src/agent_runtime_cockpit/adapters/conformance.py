...
class ConformanceResult:
    ...
    def record(self, name: str, passed: bool, reason: str = "", skipped: bool = False) -> None:
        entry = {"test": name, "result": "SKIP" if skipped else ("PASS" if passed else "FAIL"), "reason": reason}
...
def run_conformance(adapter: RuntimeAdapter, workspace: Path) -> ConformanceResult:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            if detected and not evidence:
                result.record("detect_no_false_positive", False,
    ...
    try:
        ...
        result.record("detect_returns_valid_types", valid,
                      "" if valid else "detect() returned wrong types")
        if detected and not evidence:
            result.record("detect_evidence_when_detected", False, "Detected with zero evidence items")
        else:
    ...
    if caps.can_export_workflow:
        try:
            ...
            result.record("export_workflow_returns_list", ok,
                          "" if ok else "export_workflow() returned wrong type")
            if ok and workflows:
                wf = workflows[0]
                has_nodes = len(wf.nodes) > 0
                has_entry = len(wf.entry_points) > 0
                result.record("workflow_has_nodes", has_nodes, "Workflow has no nodes" if not has_nodes else "")
                result.record("workflow_has_entry_points", has_entry, "Workflow has no entry_points" if not has_entry else "")
        except Exception as e:
            result.record("export_workflow_returns_list", False, str(e))
    else:
        try:
            adapter.export_workflow(workspace)
            result.record("export_workflow_not_implemented", False,
    ...
    if caps.can_export_schema:
        try:
            ...
            result.record("export_schemas_returns_list", ok,
    ...
    if not caps.can_run:
        ...
        try:
            ...
            result.record("run_not_implemented_when_unsupported", False,
