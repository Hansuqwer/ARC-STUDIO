...
class OpenAIAgentsAdoptionRunner(AdoptionRunner):
    ...
    def check_availability(self, workspace: Path) -> AdoptionCapability:
        try:
            ...
            return AdoptionCapability(
                ...
                doctor_actions=[{"id": "install_agents", "label": "Install OpenAI Agents SDK", "description": "pip install openai-agents", "command": "pip install openai-agents"}],
            )
    ...
    async def run(self, spec: AdoptionSpec, run_id: str, emit_event) -> ConsensusResult:
        ...
        proposal = WorkerProposal(task_id="openai-agent", worker_id=getattr(agent, "name", "openai-agent"), output=output, confidence=1.0, metadata={"runtime": "openai-agents", "swarmgraph_role": "coder"})
