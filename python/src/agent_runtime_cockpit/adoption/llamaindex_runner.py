...
class LlamaIndexAdoptionRunner(AdoptionRunner):
    ...
    def check_availability(self, workspace: Path) -> AdoptionCapability:
        try:
            if importlib.util.find_spec("llama_index") is None:
                raise ImportError("llama_index")
            _setup_swarmgraph_paths()

            return AdoptionCapability(
                mode=self.mode,
                status=AdoptionStatus.RUNNABLE,
                reason="LlamaIndex package detected; SwarmGraph adoption path is fake-tested/gated",
            )
        except ImportError:
            return AdoptionCapability(
                mode=self.mode,
                ...
                doctor_actions=[
                    {
                        "id": "install_llamaindex",
                        "label": "Install LlamaIndex",
                        "description": "pip install llama-index",
    ...
    async def run(self, spec: AdoptionSpec, run_id: str, emit_event) -> ConsensusResult:
        target = (
            spec.runtime_config.get("workflow")
            or spec.runtime_config.get("query_engine")
            or spec.runtime_config.get("agent")
        )
        if target is None:
            raise ValueError(
                "LlamaIndex adoption requires workflow/query_engine/agent in runtime_config"
            )
        ...
        proposal = WorkerProposal(
            task_id="llamaindex-query",
            worker_id="llamaindex",
            output=output,
            confidence=1.0,
            metadata={"runtime": "llamaindex", "swarmgraph_role": "researcher"},
        )
        emit_event(
            run_id,
            "WORKER_COMPLETED",
