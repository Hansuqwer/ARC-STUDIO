...
if git ls-files | grep -E '(^|/)(\.venv|\.venv2|node_modules|dist|lib|src-gen|test-results)(/|$)|(^|/)\.cache/|^python/\.arc/traces/.*\.jsonl$|^applications/.*/(gen-webpack(\.node)?\.config|webpack\.config)\.js$|(^|/)\.env(\..*)?$|\.(pem|key)$|(^|/)id_rsa'; then
