...
from src.local_config import load_config, get_models_by_tags, get_cost
...
import tiktoken
from src.local_scores import get_leaderboard
...
# Stub Amplitude (removed dependency)
class Amplitude:
    def __init__(self, api_key):
        pass

    def track(self, event):
        logger.debug(f"Amplitude stub: {event}")


class BaseEvent:
    def __init__(self, event_type, user_id=None, event_properties=None):
        self.event_type = event_type
        self.user_id = user_id
        self.event_properties = event_properties or {}


AMPLITUDE_API_KEY = os.environ.get("AMPLITUDE_API_KEY", "stub")
...
async def timed_create(client: IBaseClient, state, model, options):
    try:
        ...
        MODEL_CREATE_LATENCY.labels(client=client.__class__.__name__, model=model).observe(latency)
...
async def timed_create_test(client, state, model, options):
    ...
    result = await asyncio.sleep(random.uniform(0.5, 1.5))
    result = "test"
    end_time = asyncio.get_event_loop().time()
    latency = end_time - start_time
    print(
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if isinstance(e, ModelTimeoutError):
                    error_msg = (
                        f"Service temporarily unavailable due to timeout for model: {e.model}"
                    )
...
class FastAPIApp:
    def __init__(self):
        ...
        # Stub user repository (Firebase dependency removed)
        self.user_repo = None
        ...
        # Initialize global outcomes from SQLite
        self.update_global_outcomes()

        self.setup_routes()
        logger.info("API is starting up")

    def update_global_outcomes(self):
        """Load all outcomes from SQLite for ELO computation."""
        try:
            self.global_outcomes = self.sqlite_client.get_all_outcomes()
            logger.info(f"Loaded {len(self.global_outcomes)} outcomes from SQLite")
        except Exception as e:
            logger.error(f"Failed to load outcomes from SQLite: {str(e)}")
    ...
    def select_models(self, tags):
        ...
        if len(tagged_models) == 0:
            raise HTTPException(status_code=500, detail="Error in tagged models")
        ...
        if total_prob == 0:
            raise HTTPException(status_code=500, detail="Distribution probabilities sum to 0")
        normalized_probs = [p / total_prob for p in probabilities]

        # Select a pair based on the normalized distribution
        selected_pair = random.choices(valid_pairs, weights=normalized_probs, k=1)[0]
    ...
    def setup_routes(self):
        ...
        async def create_edit_pair(request: Request, background_tasks: BackgroundTasks):
            ...
            code_to_edit_tokens = self.encoding.encode(state.prefix, allowed_special="all")
            ...
            (response1, latency1), (response2, latency2) = await asyncio.gather(task1, task2)
            ...
            latency_breakdown["response_preparation"] = time.time() - response_prep_start
            ...
            try:
                if isinstance(prompt1, str):
                    ...
                    for msg in prompt1:
                        content = msg["content"]
                        prompt1_tokens.extend(self.encoding.encode(content, allowed_special="all"))
                ...
                if isinstance(prompt2, str):
                    ...
                    for msg in prompt2:
                        content = msg["content"]
                        prompt2_tokens.extend(self.encoding.encode(content, allowed_special="all"))
            ...
            response1_tokens = self.encoding.encode(response1.text, allowed_special="all")
            response2_tokens = self.encoding.encode(response2.text, allowed_special="all")
            ...
            background_tasks.add_task(
                self.sqlite_client.upload_completion,
                responseItem1,
            )
            background_tasks.add_task(
                self.sqlite_client.upload_completion,
        ...
        async def create_pair(request: Request, background_tasks: BackgroundTasks):
            ...
            (completion1, latency1), (completion2, latency2) = await asyncio.gather(task1, task2)
            latency_breakdown["task_execution"] = time.time() - task_execution_start
            latency_breakdown["model1_latency"] = latency1
            latency_breakdown["model2_latency"] = latency2
            ...
            completion1_tokens = self.encoding.encode(completion1.text, allowed_special="all")
            completion2_tokens = self.encoding.encode(completion2.text, allowed_special="all")
        ...
        async def add_completion(request: Request, background_tasks: BackgroundTasks):
            try:
                ...
                try:
                    PrivacySetting(data.get("privacy"))
                except ValueError:
                    error_msg = (
                        f"Invalid privacy value. Must be one of: {', '.join(PrivacySetting)}"
                    )
                    logger.info(error_msg)
                    ERROR_COUNT.labels(
                        method="POST",
                ...
                background_tasks.add_task(self.sqlite_client.upload_outcome(data))
            except HTTPException:
                raise
            except Exception as e:
        ...
        async def add_single_outcome(request: Request):
            try:
                ...
                self.sqlite_client.upload_outcome(data)
            except HTTPException:
                raise
            except Exception as e:
                error_msg = f"An error occurred: {str(e)}\n\nTraceback:\n{traceback.format_exc()}"
        ...
        @self.limiter.limit("1000/minute")
        async def add_completion_outcome(request: Request, background_tasks: BackgroundTasks):
            try:
                try:
                    data = await request.json()
                except JSONDecodeError:
                ...
                for completion_item in completion_items:
                    if not all(key in completion_item for key in required_completion_keys):
                        ERROR_COUNT.labels(
                            method="POST",
                            endpoint="/add_completion_outcome",
                            exception="BadRequest",
        ...
        async def get_user_scores(request: Request):
            try:
                ...
                try:
                    user_outcomes = self.sqlite_client.get_user_outcomes(user_id)
                    ...
                    self.user_scores_cache[user_id] = leaderboard
        ...
        async def get_user_vote_count(request: Request):
            try:
                ...
                user_id = data.get("userId")
                user_outcomes = self.sqlite_client.get_user_outcomes(user_id)
                user_vote_count = len(user_outcomes)
        ...
        async def authenticate_user(user: User):
            ...
            except (UserNotFoundError, PasswordIncorrectError):
