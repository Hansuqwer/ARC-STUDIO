...
def test_model_distribution(fast_app, n_trials):
    ...
    for is_edit in [False, True]:
        ...
        for model in sorted(expected_probs.keys()):
            ...
            print(
                f"{model:<30} {expected_prob:>11.4f} {observed_prob:>11.4f} {diff_percent:>+9.1f}%"
            )
...
def test_tag_filtering(fast_app):
    ...
    for _ in range(100):
        ...
        assert all(model in edit_models for model in models), (
            f"Selected models {models} don't all have edit tag"
        )

    # Test with autocomplete (no edit tag)
    autocomplete_models = get_models_by_tags([], fast_app.models, fast_app.tag_to_models)
    for _ in range(100):
        models, _, _ = fast_app.select_models(tags=[])
        assert all(model in autocomplete_models for model in models), (
...
def test_empty_tags_uses_all_models(fast_app):
    ...
    assert all_models == set(fast_app.models), (
...
def test_model_client_mapping(fast_app):
    ...
    for _ in range(100):
        ...
        assert models[0] in client1.models, f"Client 1 doesn't support model {models[0]}"
        assert models[1] in client2.models, f"Client 2 doesn't support model {models[1]}"
...
def test_model_position_distribution(fast_app, n_trials):
    ...
    for model in sorted(position_counts.keys()):
        ...
        print(f"{model:<30} {first_percent:>11.1f} {second_percent:>11.1f} {diff_percent:>+9.1f}")
