...
def get_battle_df(outcomes_df, incl_models, interval_size):
    ...
    try:
        outcomes_df = (
            ...
            .loc[lambda df: df["model_a"].isin(incl_models) & df["model_b"].isin(incl_models)]
...
def process_completions(completions_df, timestamp_to_interval, max_outcome_timestamp):
    ...
    for key, value in pairs.items():
        if len(value) == 2:
            timestamp = max(value[0][2], value[1][2])  # Use the later timestamp of the pair
            interval = assign_interval(timestamp, timestamp_to_interval, max_outcome_timestamp)
...
def get_coefficients(df, lam, model_list, base=10):
    ...
    ptbl_win = ptbl_win.fillna(0)  # what is the effect of this? a better conditioned matrix??
...
def compute_mle_elo(
    ...
    def get_loss_for_lambda(lam, train_user_df, test_user_rows):
        ...
        sample_weights = np.concatenate((global_sample_weights, user_sample_weights), axis=0)
...
def get_interval_group(battles_df, start_interval, end_interval):
    ...
    """
    return battles_df[
        (battles_df["interval"] >= start_interval) & (battles_df["interval"] <= end_interval)
    ]
...
def get_scores(
    ...
    user_battles = get_battle_df(user_outcomes_df, incl_models=models, interval_size=interval_size)
    ...
    model_votes = pd.concat([user_battles["model_a"], user_battles["model_b"]]).value_counts()
...
def main():
    ...
    global_outcomes_df = gcp_client.get_outcomes_df(bucket_name=bucket_name, file_path=file_path)
    ...
    sample_user_df = global_outcomes_df.head(100)  # Just using first 100 rows as example
