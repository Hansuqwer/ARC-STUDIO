...
class MistralClient(IBaseClient):
    ...
    async def create(
        ...
        completion = parse_response(
            ...
            options.max_lines,
            stop_tokens,
            generate_start_phrases_for_model(self._prompt_generators, options.prompt_index, model),
            state,
            get_prompt_options_for_model(self._prompt_generators, options.prompt_index, model),
        )
    ...
    def generate_prompt_for_model(self, state: State, model: str, prompt_index: int):
        return generate_prompt_for_model(self._prompt_generators, prompt_index, state, model)

    def generate_stop_tokens_for_model(self, model: str, prompt_index: int):
        return generate_stop_tokens_for_model(self._prompt_generators, prompt_index, model)


async def main():
    client = MistralClient()
    state = State(prefix="def is_odd(n): \n return n % 2 == 1 \ndef test_is_odd():", suffix="")
    prompt_index = 1
    options = LLMOptions(
        temperature=0.5,
        max_tokens=1024,
