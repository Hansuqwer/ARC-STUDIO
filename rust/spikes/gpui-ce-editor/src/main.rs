//! gpui-ce spike — same harness pattern as gpui spike (on_next_frame chaining,
//! pure render, no stdout/stderr in run loop). gpui-ce is API-compatible.
...
const VERSION: &str = "rev c237d57d";
    ...
    if finished {
        view.update(cx, |v, _cx| {
            ...
            if let Ok(report) = assemble_report(CANDIDATE, VERSION, &results, &v.cfg) {
                let _ = report.write(std::path::Path::new("../../reports/spike-gpui-ce.json"));
                let blockers = report.spike_verdict().err().unwrap_or_default();
                let _ = std::fs::write(
                    "../../reports/spike-gpui-ce.status",
                    format!("done\nblockers:\n{}", blockers.join("\n")),
                );
            }
        });
    ...
    gpui_platform::application().run(|cx: &mut App| {
        ...
        cx.open_window(options, |window, cx| {
            let view = cx.new(|_cx| SpikeView {
                ...
                lines: vec!["gpui-ce spike — starting…".into()],
                done: false,
                cfg: config(),
            });
