...
/// Wire this loop into floem's single window/event loop. Call
/// `script.on_present(Instant::now())` from a trustworthy paint/frame callback,
/// apply the returned action to the next frame, then request paint once.
    ...
    loop {
        match script.on_present(Instant::now()) {
            Action::OpenWorkload { path, label } => {
                todo!("G1: {label}; read {path:?}, swap text view, request paint, wait present")
            }
            Action::LoadDiff { path } => todo!("G2 setup: read {path:?}, swap diff view"),
            Action::ScrollStep => todo!("G2: scroll diff one scripted step, request paint once"),
            Action::AppendRows { from, count } => todo!(
                "G3: append rows [{from}..{}) this frame only, request paint once",
                from + count
            ),
            Action::TypeChar { ch } => todo!("G4: insert synthetic key {ch:?}, request paint once"),
            Action::TakeScreenshot { out } => {
                todo!("G7: render bidi/ligature view and write screenshot to {out:?}")
            }
            Action::Settle => todo!("settle frame: no sampled mutation, request paint once"),
