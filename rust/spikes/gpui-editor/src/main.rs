...
/// Wire this loop into gpui's single window/event loop. Call
/// `script.on_present(Instant::now())` from `Window::on_next_frame`, apply the
/// returned action to the view for the next frame, then request/refresh once.
fn run_candidate_script(mut script: FrameScript) -> anyhow::Result<ScriptResults> {
    loop {
        match script.on_present(Instant::now()) {
            Action::OpenWorkload { path, label } => todo!(
                "G1: {label}; read {path:?}, swap text view, refresh, wait for next gpui on_next_frame"
            ),
            Action::LoadDiff { path } => {
                todo!("G2 setup: read {path:?}, swap diff view, warm up through Settle actions")
            }
            ...
            Action::TypeChar { ch } => todo!(
                "G4: insert synthetic key {ch:?}, refresh once, next present closes sample"
            ),
