...
fn main() -> std::io::Result<()> {
    let out = std::path::PathBuf::from(
        std::env::args()
            .nth(1)
            .unwrap_or_else(|| "spike-workloads".into()),
    );
    ...
    for (name, content) in [
        (
            "g1-source-10mb.txt",
            source_like(10 << 20, seeds::G1_SOURCE),
        ),
        (
            "g1-source-100mb.txt",
            source_like(100 << 20, seeds::G1_SOURCE),
        ),
        (
            "g1-pathological-10mb-single-line.txt",
            pathological_single_line(10 << 20, seeds::G1_PATHOLOGICAL),
        ),
        (
            "g2-diff-5k-lines.patch",
            synthetic_diff(5000, seeds::G2_DIFF),
        ),
    ] {
        ...
        digests.insert(
            name.into(),
            serde_json::json!(format!("{:016x}", fnv1a(content.as_bytes()))),
        );
