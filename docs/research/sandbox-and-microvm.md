...
Status: research complete for P0 sandbox foundation. MicroVM remains default-off. Linux/Firecracker is a gated scaffold with no live Firecracker boot/run/teardown proof on this host. macOS direct Apple VZ has real gated evidence: `arc sandbox run --provider microvm -- pwd` booted a no-NIC guest, proved no-network/workspace/symlink/teardown/audit, and returned guest stdout `/workspace`. This is proof of the gated VZ path for guest-available argv only, not production-grade or arbitrary host-command microVM execution.
...
| Lima network docs | https://lima-vm.io/docs/config/network/ | Lima has default/named networking modes; docs do not provide a strict no-network proof for ARC's public sandbox contract. | Doctor keeps macOS Lima as low-security harness and reports Lima public execution blocked even if `limactl` is installed. | High | Direct VZ now supplies the strict no-network candidate; production-grade/broad execution still needs repeatable host CI and artifact policy. |
...
| Lima VZ docs | https://lima-vm.io/docs/config/vmtype/vz/ | VZ uses macOS Virtualization.framework, requires macOS >=13.0, default since Lima v1.0, and does not support cross-arch whole VM. | Doctor/preflight reports Lima/VZ as low-security harness; direct Apple VZ is separate from Lima. | High | Direct Apple VZ gated public path now exists outside Lima; Lima remains low-security. |
...
| Lima network docs | https://lima-vm.io/docs/config/network/ | Lima docs describe default/named network choices and no documented strict no-network template key. | Preserve macOS Lima/VZ as low-security harness only; no Lima public execution claim. | High | Direct Apple VZ gated public path now exists for guest-available `pwd`. |
...
| Lima docs | https://lima-vm.io/docs/ and https://lima-vm.io/docs/config/vmtype/vz/ | Lima launches Linux VMs on macOS; VZ uses Apple Virtualization.framework and requires macOS >= 13, but Lima networking is present by default. | Keep macOS Lima path as low-security harness/template only; no strict public execution claim from Lima. | High | Direct Apple VZ gated public path now exists outside Lima for guest-available `pwd`. |
...
| Lima default user-mode network docs | https://lima-vm.io/docs/config/network/user/ | Lima enables user-mode slirp by default on `192.168.5.0/24`, with guest/host/DNS addresses. | macOS Lima remains low-security/network-present harness only; strict Lima public microVM execution remains blocked. | High | Direct Apple VZ gated public path now exists; future Lima no-network support would be a separate option. |
...
| VZ host proof | `cd python && ARC_VZ_PROOF=1 ARC_VZ_RUNNER=/var/folders/dp/1fh07k_922j5qk7xfncn1zv40000gn/T/opencode/arc-vz-runner ARC_VZ_KERNEL=/var/folders/dp/1fh07k_922j5qk7xfncn1zv40000gn/T/opencode/arc-vz-proof/debian-linux ARC_VZ_INITRD=/var/folders/dp/1fh07k_922j5qk7xfncn1zv40000gn/T/opencode/arc-vz-proof/arc-vz-proof-initrd.gz ARC_VZ_TIMEOUT_SECONDS=45 uv run pytest tests/isolation/test_vz_proof.py -v` | On macOS 26.4 arm64, the compiled/signed Swift runner booted a VZ Linux guest with `networkDevices=[]`; guest markers proved boot, no guest ethernet, no default route, network tool available, network probe failure, workspace mount, sentinel read, symlink escape blocked, command result markers, and teardown ok. | Direct VZ is a real opt-in host-proof path, not just source scaffold. Current public CLI path additionally requires exact argv hash and all VZ gates. | High | Need durable runner/artifact generation, opt-in CI host, artifact policy, and broader failure-mode tests before production-grade claims. |
| VZ artifact provenance | `cd python && uv run arc sandbox vz-artifacts --json --output /var/folders/dp/1fh07k_922j5qk7xfncn1zv40000gn/T/opencode/arc-vz-artifacts --kernel /var/folders/dp/1fh07k_922j5qk7xfncn1zv40000gn/T/opencode/arc-vz-proof/debian-linux --initrd /var/folders/dp/1fh07k_922j5qk7xfncn1zv40000gn/T/opencode/arc-vz-proof/arc-vz-proof-initrd.gz --build-runner` | The CLI copied runner source, entitlements, kernel, initrd; compiled and ad-hoc signed the runner; wrote `vz-artifacts-manifest.json` with SHA256 hashes, tool paths, build commands, `networkDevices=[]`, `public_execution_enabled=false`, and no blockers. | Durable local artifact/provenance generation is real and does not boot a VM or download assets. Public macOS microVM execution remains blocked. | High | Kernel/initrd provenance beyond local hashes, artifact distribution, and CI retention remain future work. |
| VZ static-proof blocker | `ARC_MICROVM_EXEC_ENABLED=1 ARC_MICROVM_INTEGRATION=1 ARC_VZ_REAL_EXEC=1 ARC_VZ_ARTIFACT_MANIFEST=... uv run arc sandbox run --json --provider microvm --policy local-safe -- python -c "print('hello-vz')"` against the original proof initrd | The original initrd ignored the requested argv and emitted static `arc-vz-proof-ok`; after the host required `ARC_VZ_RESULT command_sha256=<sha256(argv-json)>`, the run correctly denied with `guest command result did not prove requested argv hash`. | Public VZ success must require guest proof of the exact requested argv hash. Static proof-only initrds cannot satisfy execution. | High | Guest artifacts must contain the requested binary/runtime; `python -c` is not proven unless Python is present inside the guest. |
| VZ gated public run proof | `cd python && ARC_MICROVM_EXEC_ENABLED=1 ARC_MICROVM_INTEGRATION=1 ARC_VZ_REAL_EXEC=1 ARC_VZ_ARTIFACT_MANIFEST=/var/folders/dp/1fh07k_922j5qk7xfncn1zv40000gn/T/opencode/arc-vz-artifacts-exec/vz-artifacts-manifest.json ARC_VZ_TIMEOUT_SECONDS=45 uv run arc sandbox run --json --provider microvm --policy local-safe -- pwd` | On macOS 26.4 arm64, the signed Swift runner booted a VZ guest with `networkDevices=[]`, passed `ARC_VZ_COMMAND_ARGV_B64CSV` and `ARC_VZ_COMMAND_SHA256`, mounted workspace at `/workspace`, proved no guest ethernet, no default route, failed network probe, sentinel read, symlink escape blocked, exact argv hash, teardown ok, and emitted sandbox audit. Guest stdout was `/workspace`. | `arc sandbox run --provider microvm` is no longer purely blocked on macOS when all three env gates and valid local VZ manifest are present. It remains default-off and limited to commands available inside the guest artifact; no Lima, Docker Desktop, Windows, or production-grade claim. | High | Need repeatable host CI, broader argv/runtime coverage, SIGINT failure evidence, artifact distribution/license policy. |
| VZ exec-init contract/initrd packer | `cd python && uv run arc sandbox vz-artifacts --json --exec-init --output /tmp/arc-vz-exec-init`; optional `--pack-initrd --busybox /path/to/busybox` | ARC writes `arc-vz-exec-init.sh` plus `vz-exec-init-manifest.json` with no downloads, no shell-string execution, explicit argv transport, command hash marker, guest requirements, and `python_runtime_included=false`. With `--pack-initrd`, ARC packages a gzip `newc` initramfs from that script and an operator-supplied local BusyBox using local `cpio`; normal CI fakes the pack step and covers missing-tool/missing-BusyBox blockers. | Durable artifact policy is reviewable and can produce a local initrd shell when local tools are supplied. The generated initrd still does not include Python or arbitrary guest runtimes, and has not replaced the proven host artifact until a gated VZ run uses it. | High | Need real host proof using the packed initrd, plus license/provenance for kernel/BusyBox/runtime contents. |
| VZ packed-initrd host-proof attempt | `cd python && uv run arc sandbox vz-artifacts --json --exec-init --pack-initrd --busybox /var/folders/dp/1fh07k_922j5qk7xfncn1zv40000gn/T/opencode/arc-vz-busybox-extract/usr/bin/busybox --output /var/folders/dp/1fh07k_922j5qk7xfncn1zv40000gn/T/opencode/arc-vz-packed-initrd-attempt` | The only local BusyBox candidate extracted from the prior Debian initrd is ARM64 but dynamically linked (`/lib/ld-linux-aarch64.so.1`). ARC now blocks it with `ARC_VZ_BUSYBOX must be a static Linux BusyBox binary`; no packed initrd or VM proof was produced. | The packer is fail-closed for standalone initramfs correctness. A real host proof needs an operator-supplied static ARM64 Linux BusyBox or a fuller initramfs/rootfs policy that includes loader/libs. | High | Need static BusyBox provenance/distribution decision before packed-initrd host proof. |
| VZ host proof CI lane | `.github/workflows/vz-host-proof.yml` | Added a manual `workflow_dispatch` lane for self-hosted macOS ARM64 runners only. It requires operator-supplied local kernel and static BusyBox paths, builds the packed initrd and signed runner/provenance manifest, then runs gated `arc sandbox run --provider microvm -- pwd` plus the host-gated pytest smoke. | Repeatable proof wiring exists without burdening normal CI or downloading guest artifacts. It is not evidence until run on an eligible self-hosted runner with valid local artifacts. | Medium | Need a maintained self-hosted macOS runner and artifact retention policy. |
| VZ public host smoke scaffold | `python/tests/isolation/test_vz_proof.py::test_vz_public_real_host_pwd` | Host-gated test runs only on Darwin when all three public VZ gates and `ARC_VZ_ARTIFACT_MANIFEST` are set. Normal CI skips. | Repeatable local/CI hook exists for the proven `pwd` path without requiring VZ on normal CI. | High | Need hosted macOS runner with local artifacts to run it repeatedly. |
...
| Direct VZ host proof | Keep `VZNoNetworkProof.run_proof()` as a proof diagnostic; wire public VZ only behind explicit env/manifest/proof/hash gates | Claim proof from config only; use Lima as strict substitute; allow static proof-only initrd success | Host proof boots and tears down a no-NIC VZ guest. Public CLI success now additionally requires exact requested argv hash, teardown, audit, and a valid manifest; static proof-only output is denied. | `python/src/agent_runtime_cockpit/isolation/vz_provider.py`, `python/tests/isolation/test_vz_proof.py`, `tools/arc-vz-runner.swift`, docs | High |
| Direct VZ artifact provenance | Add `arc sandbox vz-artifacts`; copy/build/sign local assets and write SHA256 manifest; do not download or boot | Keep temp-only runner path; embed binary assets in repo; auto-download kernel/initrd | Gives repeatable local proof inputs without introducing opaque downloads or public execution. | `python/src/agent_runtime_cockpit/isolation/vz_provider.py`, `python/src/agent_runtime_cockpit/cli/sandbox.py`, `python/tests/isolation/test_vz_proof.py`, `python/tests/test_cli_sandbox.py`, docs | High |
| macOS VZ public execution gate | Wire `MicroVMIsolationProvider.execute()` to direct Apple VZ only when `ARC_MICROVM_EXEC_ENABLED=1`, `ARC_MICROVM_INTEGRATION=1`, `ARC_VZ_REAL_EXEC=1`, valid `ARC_VZ_ARTIFACT_MANIFEST`, signed runner, readable kernel/initrd, no-NIC source, guest proof markers, exact argv hash, teardown, and audit all pass | Lima strict proof, Docker Desktop-as-microVM, broad shell command execution, static proof-only initrd success | Meets the user's macOS evidence requirement with direct Apple Virtualization.framework only, remains default-off, and fails closed if the guest cannot prove the requested argv. | `python/src/agent_runtime_cockpit/isolation/vz_provider.py`, `python/src/agent_runtime_cockpit/isolation/microvm.py`, `tools/arc-vz-runner.swift`, `python/tests/isolation/test_vz_proof.py`, `python/tests/test_cli_sandbox.py`, docs | Medium |
| VZ exec-init artifact policy | Commit a reviewable init script + manifest generator; optionally package a local-tool initrd only when the operator supplies static Linux BusyBox and local `cpio`; never download kernel/initrd/runtime assets | Auto-download distro initrd; commit opaque binary initrd; accept dynamically linked BusyBox without loader/libs; claim Python runtime from host command; require initrd packaging by default | Keeps provenance explicit and prevents accidental `python -c` claims. The manifest states guest requirements, `python_runtime_included=false`, static BusyBox requirement/status, local BusyBox hash, initrd hash, and pack blockers. | `python/src/agent_runtime_cockpit/isolation/vz_provider.py`, `python/src/agent_runtime_cockpit/cli/sandbox.py`, tests, docs, `.github/workflows/vz-host-proof.yml` | High |
...
| Phase 105 Linux execution path | Wire Linux/Firecracker execution behind dual public/integration gates plus real host gates; keep macOS Lima blocked and direct VZ separate | Keep permanent `NotImplementedError`; claim proof from fake runner; use Lima as fallback | Firecracker can be configured no-NIC by construction, and public execution can fail closed unless guest proof/result markers are present. Tests remain skipped off Linux/KVM. | `python/src/agent_runtime_cockpit/isolation/microvm.py`, `python/src/agent_runtime_cockpit/cli/sandbox.py`, `python/tests/isolation/test_firecracker_smoke.py`, `python/tests/isolation/test_microvm_truth_guard.py`, docs | Medium |
...
- direct Apple VZ gated public path exists for `arc sandbox run --provider microvm` only when `ARC_MICROVM_EXEC_ENABLED=1`, `ARC_MICROVM_INTEGRATION=1`, `ARC_VZ_REAL_EXEC=1`, and a valid `ARC_VZ_ARTIFACT_MANIFEST` are present. One host run proved guest-available `pwd` execution with stdout `/workspace`, exact argv hash, no NIC/default route, failed network probe, workspace/symlink proof, teardown ok, and sandbox audit. This is not production-grade microVM execution and not arbitrary host-command parity; guest artifacts must contain the requested command/runtime.
- `arc sandbox vz-artifacts --exec-init --output <dir> --json` writes a reviewable VZ guest exec-init script and manifest. `--pack-initrd --busybox <path>` additionally packages a gzip `newc` initramfs from static local BusyBox and local `cpio`. It records no-download/no-shell-string policy, static BusyBox requirement/status, and guest requirements. It does not include Python or arbitrary guest runtimes.
- `arc sandbox vz-artifacts --output <dir> --kernel <path> --initrd <path> --build-runner --json` creates a proof-only local VZ artifact set, compiles/signs the runner when requested, copies kernel/initrd, and records SHA256 provenance in `vz-artifacts-manifest.json`; it does not download assets or boot a VM.
...
- production-grade or broad arbitrary-command macOS microVM execution
- real host proof using the newly packed VZ exec initrd; normal CI covers generation/failure blockers only
...
- Phase 104 direct Apple VZ strict no-network gated public CLI proof passed once for guest-available `pwd`; production-grade/arbitrary-command macOS microVM execution remains design-only. Lima remains low-security/network-present because default user-mode/slirp networking is documented and no no-network template key was found.
...
- macOS production-grade/broad microVM execution requires a gated host proof using the packed initrd, stable artifacts, repeated host CI, real timeout/SIGINT/failure proof, broader argv/runtime coverage, and kernel/initrd/BusyBox provenance/distribution/license policy.
- current packed-initrd proof is blocked on this host because the available ARM64 BusyBox candidate is dynamically linked; ARC requires static Linux BusyBox for the minimal standalone initramfs path.
- macOS VZ proof assets are locally hash-pinned by `arc sandbox vz-artifacts`; upstream kernel/initrd source provenance, distribution, retention, and CI artifact policy remain unresolved.
...
## VZ Artifact Provenance Policy

Status: policy defined; enforcement is local hash/provenance only; no bundled third-party artifacts are committed.

- Kernel, initrd, and static BusyBox are operator-supplied local artifacts. ARC does not download or vendor these assets.
- `arc sandbox vz-artifacts` copies kernel/initrd inputs into an output directory and records SHA256 hashes, source paths, host OS/arch, tool paths, and build commands in `vz-artifacts-manifest.json`.
- `arc sandbox vz-artifacts --exec-init --pack-initrd --busybox <path>` requires a static Linux BusyBox binary for the minimal standalone initramfs. Dynamically linked BusyBox is rejected unless a future policy also packages and hashes the loader/libs.
- Static BusyBox provenance must include upstream project/source version, build recipe or vendor source, license, target architecture, static-link confirmation, SHA256, and retention location outside the repo.
- Kernel/initrd provenance must include upstream project/source version, build recipe or vendor image source, license, target architecture, SHA256, and retention location outside the repo.
- CI must not fetch these artifacts implicitly. The manual `.github/workflows/vz-host-proof.yml` lane accepts local self-hosted runner paths only.
- A VZ host proof is valid only when the manifest used by `ARC_VZ_ARTIFACT_MANIFEST` points to the exact kernel/initrd/runner hashes used by the run and guest output proves no-network/workspace/argv-hash/teardown markers.
- `python_runtime_included=false` remains mandatory until a separate guest runtime artifact policy bundles Python and a gated host proof demonstrates `python -c` inside the guest.

## Phase 37.6 MicroVM Execution Blocker Detail

Status: macOS direct VZ strict no-network gated public CLI proof passed once for guest-available `pwd` behind `ARC_MICROVM_EXEC_ENABLED=1`, `ARC_MICROVM_INTEGRATION=1`, `ARC_VZ_REAL_EXEC=1`, and valid `ARC_VZ_ARTIFACT_MANIFEST`; production-grade/arbitrary-command macOS microVM execution remains unclaimed. Linux/Firecracker gated scaffold exists behind explicit host gates, but no real Firecracker boot proof has run on this macOS host.
...
### Network-Off Proof
- Direct Apple VZ gated public CLI path proved strict guest no-network once on this macOS host for guest-available `pwd`.
- Original `ARC_VZ_PROOF=1` proof-only path remains available for host proof diagnostics, but public CLI success now requires `ARC_VZ_RESULT command_sha256=<sha256(argv-json)>`.
...
- Current strict proof status: direct macOS VZ gated public CLI path passed once for `pwd`; Linux/Firecracker remains scaffold/host-unproven. No real Firecracker boot was executed on this macOS host.
...
| MicroVM truth guard | Updated | Default execution blocked; Linux/Firecracker all-gates path delegates to runner; macOS direct VZ all-gates path delegates to VZ runner and still fails closed without guest proof/hash/teardown/audit; status includes ADR-024 and gate state |
...
| Direct Apple VZ gated public CLI proof | Passed once for guest-available `pwd` on macOS 26.4 arm64 | Opt-in CI host, reviewed initrd builder/artifact policy, timeout/SIGINT/failure proofs, broader argv/runtime coverage |
| Direct Apple VZ artifact provenance | Real local CLI artifact generator | Upstream kernel/initrd provenance, distribution/retention/license policy, opt-in CI host |
...
| Harness/public audit events | Real for internal harnesses, Linux/Firecracker scaffold, and one macOS direct VZ gated public run | `MICROVM_COMMAND`/`MICROVM_DENIED` persisted for Lima/Firecracker harness attempts and Linux/Firecracker gated scaffold attempts; macOS direct VZ gated public run emitted `sandbox.microvm.run` audit with gates, manifest path, artifact hashes, proof markers, teardown, and exact argv proof |
| Production-ready claim | Not claimed | Would need repeated host CI, failure proofs, artifact policy, and broader guest runtime coverage |
