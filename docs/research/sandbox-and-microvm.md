...
| Direct Apple VZ host proof | Passed once behind `ARC_VZ_PROOF=1` on macOS 26.4 arm64 | Opt-in CI host, public provider wiring, audit/output caps, timeout/SIGINT cleanup, and broader failure-mode tests |
| Direct Apple VZ artifact provenance | Real local CLI artifact generator | Upstream kernel/initrd provenance, distribution/retention policy, opt-in CI host, public provider wiring |
