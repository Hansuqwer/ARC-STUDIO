...
| Lima VZ docs | https://lima-vm.io/docs/config/vmtype/vz/ | VZ uses macOS Virtualization.framework, requires macOS >=13.0, default since Lima v1.0, and does not support cross-arch whole VM. | Doctor/preflight reports Lima/VZ as low-security harness; macOS public execution remains blocked. | High | Apple VZ direct no-network option needs separate implementation/research. |
...
| Lima default user-mode network docs | https://lima-vm.io/docs/config/network/user/ | Lima enables user-mode slirp by default on `192.168.5.0/24`, with guest/host/DNS addresses. | macOS Lima remains low-security/network-present harness only; strict public microVM execution remains blocked. | High | Direct Apple VZ no-NIC helper or future Lima no-network support still needed. |
...
- direct Apple VZ no-NIC source scaffold exists (`tools/arc-vz-runner.swift`) and VZ preflight reports `networkDevices=[]`, but `strict_no_network_proof` remains `not_proven`; `proof_status=ready_to_attempt` only when `ARC_VZ_PROOF=1`, executable `ARC_VZ_RUNNER`, readable kernel, and readable initrd are present.
...
- macOS strict microVM execution
...
- macOS strict microVM execution requires a direct Apple Virtualization.framework no-NIC provider/helper or future Lima no-network support.
- macOS strict microVM proof requires compiling/running the direct VZ helper with kernel/initrd and collecting guest no-NIC/no-default-route/workspace/symlink/teardown evidence; helper source alone is not proof.
