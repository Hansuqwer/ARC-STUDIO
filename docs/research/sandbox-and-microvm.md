...
| VZ exec-init artifact policy | Commit a reviewable init script + manifest generator, but do not auto-package/download kernel/initrd/runtime assets | Auto-download distro initrd; commit opaque binary initrd; claim Python runtime from host command | Keeps provenance explicit and prevents accidental `python -c` claims. The manifest states guest requirements and `python_runtime_included=false`. | `python/src/agent_runtime_cockpit/isolation/vz_provider.py`, `python/src/agent_runtime_cockpit/cli/sandbox.py`, tests, docs | High |
...
- `arc sandbox vz-artifacts --exec-init --output <dir> --json` writes a reviewable VZ guest exec-init script and manifest. It records no-download/no-shell-string policy and guest requirements. It does not build an initrd or include Python.
...
- packaged VZ initrd generation; current `--exec-init` writes the contract script/manifest only
...
- macOS production-grade/broad microVM execution requires packaged initrd tooling/policy beyond the current exec-init contract script, stable artifacts, repeated host CI, real timeout/SIGINT/failure proof, broader argv/runtime coverage, and kernel/initrd provenance/distribution/license policy.
