...
| Firecracker guest proof channel | Proof-only serial/rootfs init markers (`ARC_FC_PROOF`) | SSH, arbitrary argv agent, host network channel | Avoids broad runtime execution. Lets host-gated proof collect P2/P3/P5 evidence when a dedicated rootfs/init agent exists. | `python/src/agent_runtime_cockpit/isolation/microvm.py`, `python/tests/isolation/test_firecracker_smoke.py` | Medium |
