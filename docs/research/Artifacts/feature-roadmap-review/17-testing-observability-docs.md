...
| **Add redaction E2E contract test** | §10.10 redaction contract spans CLI, IDE, SSE, graph. Unit tests cover redaction logic but not surface integration. | v0.1 | Medium — requires injecting secret-like values into stub run output | §10.10: Add E2E redaction verification with fake secrets across CLI, IDE, SSE, and graph inspector surfaces. |
...
6. **Redaction E2E: inject synthetic secret into stub output.** Modify the E2E runtime stub to emit a line containing a non-real secret-shaped pattern. E2E test verifies the configured redaction marker appears in IDE event stream detail (already partially tested at line 185 of `arc-smoke.spec.ts`).
...
- **§10.10 (Redaction contract):** Add paragraph covering fake secret patterns across CLI stdout, IDE chat transcript, SSE event detail, graph node inspector, run summary, and error cards. Failure to redact any pattern is a P0 release blocker.
