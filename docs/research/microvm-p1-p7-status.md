...
| P7 | Audit event emitted for every execution | **Satisfied for internal harnesses and Linux/Firecracker path; real host proof pending** | `build_microvm_audit_event()` records command, workspace, runtime, instance, lifecycle, network proof, teardown, timestamps, exit code, truncation flags, `audit_id`, and public execution status. Linux/Firecracker runner persists audit events after proof/run completion; CLI also audits blocked attempts. | Add/record real-host audit evidence when Linux/KVM proof runs. |
