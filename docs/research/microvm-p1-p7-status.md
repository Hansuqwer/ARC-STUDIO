...
|---|---|---|---|---|
| P1 | Lifecycle proof: create→start→exec→stop/delete completes | **Partial** | `test_lima_smoke.py::TestLimaSmokeRealHost` added; uses real `_run_limactl` when `ARC_LIMA_REAL_EXEC=1`; code-level lifecycle state machine verified with fake runner (105+ tests). `FirecrackerProofRunner` now has a private host-gated proof harness that writes a config, starts Firecracker as a process-group subprocess, parses proof-only guest markers, and tears down. Firecracker proof rootfs/init artifact tooling now writes deterministic init + manifest; optional ext4 build is gated. Real VM lifecycle not yet run end-to-end on this host. | Build/run the ARC Firecracker proof rootfs/init artifact on Linux/KVM. Then record host-gated proof evidence. |
| P2 | Network-off proof: guest has no default route before user argv | **Firecracker/Cloud Hypervisor proof path added; strict public microVM still BLOCKED** | Lima remains low-security/network-present. Firecracker has a no-NIC design config model that omits `network-interfaces`; Cloud Hypervisor has a no-`--net` argv/config scaffold. `FirecrackerProofRunner` creates a no-NIC config and can parse `ARC_FC_PROOF no_default_route=1` plus `curl_failed=1` markers. `generate_firecracker_proof_artifacts()` creates the marker init/manifest, but no real guest `ip route`/`curl` proof has run here. | Build rootfs/init marker image with `ARC_FC_BUILD_PROOF_ROOTFS=1` on a suitable Linux host, then run host-gated Firecracker proof and verify no default route + curl failure. Do not wire `ARC_MICROVM_EXEC_ENABLED`. |
...
Firecracker/Cloud Hypervisor are proof-path candidates only; a private Firecracker host-gated proof harness and proof-marker parser exist, but no real guest no-route/curl-fails proof has run.
Firecracker proof rootfs/init artifact tooling exists, but ext4 build and real boot are not proven on this macOS host.
