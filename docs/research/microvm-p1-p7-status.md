...
| P2 | Network-off proof: guest has no default route before user argv | **Firecracker/Cloud Hypervisor proof path added; strict public microVM still BLOCKED** | Lima remains low-security/network-present. Firecracker has a no-NIC design config model that omits `network-interfaces`; Cloud Hypervisor has a no-`--net` argv/config scaffold. Both report `strict_network_proof=not_proven` and include `ip route` + `curl --connect-timeout 2 https://example.com` proof commands. Real Linux/KVM boot was not run on this macOS host. | Run host-gated Firecracker or Cloud Hypervisor proof on Linux/KVM with binary, kernel, rootfs/disk, `ARC_MICROVM_INTEGRATION=1`, and provider-specific real-exec gate; verify no default route, curl failure, output caps, teardown. Do not wire `ARC_MICROVM_EXEC_ENABLED`. |
...
Firecracker/Cloud Hypervisor are now design/preflight candidates only; no real boot/no-route/curl-fails proof has run.
