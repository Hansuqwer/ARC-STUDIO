...
| P5 | Symlink/path-traversal escape denied in guest mount | **Partial; real-host proof scaffold added** | Code-level: `is_path_within_root()` correctly denies dangling, chained, and cross-parent symlinks (19 tests). `test_real_lima_symlink_escape_blocked` now creates a workspace symlink to `/etc/passwd`, runs `cat /workspace/arc-host-passwd-link` inside Lima, and fails if `root:` is readable. The test is skipped unless `ARC_MICROVM_INTEGRATION=1 ARC_LIMA_REAL_EXEC=1` on macOS with `limactl`. | Run `ARC_MICROVM_INTEGRATION=1 ARC_LIMA_REAL_EXEC=1 uv run pytest tests/isolation/test_lima_smoke.py::TestLimaSmokeRealHost::test_real_lima_symlink_escape_blocked -q` on a Lima host. Lima virtiofs may expose host symlink targets; if so P5 remains blocked and public microVM execution must stay disabled. |
...
4. **P5 mount-level**: Run the real-host symlink escape test inside Lima guest and record whether Lima blocks or exposes the target.
