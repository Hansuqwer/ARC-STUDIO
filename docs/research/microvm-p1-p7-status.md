...
**Platform checked:** macOS Darwin 26.4, arm64, limactl 2.1.0; direct Apple VZ gated `arc sandbox run --provider microvm -- pwd` passed once with local hash-pinned artifacts; VZ exec-init contract generator and static-BusyBox initrd packer exist; the packed-initrd path is now reproducible here via `tools/arc-vz-bringup.sh` (static BusyBox + open-source Kata `vmlinux.container` kernel + the `/proc` init fix) and the gated `pwd` proof passes; Firecracker/KVM unavailable on this host
...
|---|---|---|---|---|
| P1 | Lifecycle proof: create→start→exec→stop/delete completes | **macOS direct VZ gated public CLI proof passed once for guest-available `pwd`; packed initrd host-proven (gated, reproducible via `tools/arc-vz-bringup.sh`); Linux/Firecracker gated scaffold unproven on this host** | `arc sandbox run --provider microvm -- pwd` booted VZ, ran guest argv, returned `/workspace`, and stopped with teardown ok. `arc sandbox vz-artifacts` can compile/sign/hash-pin the runner and inputs. `arc sandbox vz-artifacts --exec-init --pack-initrd` writes the reviewable guest init contract and can package a static-BusyBox initrd, but this host only found a dynamically linked BusyBox candidate and correctly blocked packaging. Linux scaffold writes no-NIC config, creates a workspace snapshot, starts Firecracker only behind host/env gates, parses proof/result markers, terminates process group, and removes temp dir. Tests fake the Firecracker subprocess. | Add static BusyBox provenance/input, run manual VZ host CI, real SIGINT/failure proofs, and opt-in Firecracker test on Linux/KVM. |
...
contract and manifest. `--pack-initrd --busybox <path>` packages a minimal
initramfs only with static local BusyBox and local `cpio`; dynamically linked
BusyBox is blocked unless a future policy packages loader/libs. It does not
download or bundle Python/guest runtimes.
...
4. For macOS strict execution, provide static ARM64 Linux BusyBox, repeat the direct VZ public path in the manual host CI lane with the packed initrd, add real timeout/SIGINT/failure proofs, and define kernel/initrd/BusyBox distribution/provenance policy before any production-grade or broad arbitrary-command claim.
