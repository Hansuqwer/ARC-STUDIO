...
The code-execution surface is high risk. ARC integration therefore implements T1/T2 static analysis and a T3 gated scaffold only. No agent code is executed by default.
...
| Runner | `ARC_SMOLAGENTS_RUNNER_ENABLED=1` gated scaffold | Default live execution | Code execution + provider/network risk | `adapters/smolagents/runner.py` | High |
...
- No agent execution unless `ARC_SMOLAGENTS_RUNNER_ENABLED=1`.
