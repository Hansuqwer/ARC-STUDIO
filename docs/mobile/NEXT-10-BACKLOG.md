...
- **T7 ✅ (Phase 9)** React Native TurboModule scaffold — `src/NativeArcMobileRuntime.ts` (New-Arch Codegen `Spec extends TurboModule`), `package.json` `codegenConfig` (type modules), `src/index.ts` (TurboModule + fixture fallback + getCapabilities/simulate, 13-cap drift-guarded), iOS `.mm` + Android Kotlin fixture stubs. Test `test_mobile_rn.py` (7) incl. recursive forbidden-symbol scan + TS↔iOS↔Android parity. Fixtures only.
- **T8 ✅ (Phase 10 — Phases 9/10 COMPLETE)** Flutter platform interface — `lib/src/{models,platform_interface,method_channel}.dart` + `arc_mobile_runtime.dart` facade: federated `ArcMobileRuntimePlatform`, Dart models w/ JSON round-trip, method-channel fixtures fallback (MissingPluginException), 13-cap catalog. Verified with REAL `flutter analyze` (clean) + `flutter test` (5/5) + `test_mobile_flutter.py` (7). Phases 9/10 recorded as main Phase 189.
- **T9 ✅ (Phase 12)** SIEM export — `mobile/siem_export.py` (`export_trace_cef`/`export_trace_json`/`export_trace`) + `arc mobile siem-export --format cef|json` CLI: deterministic CEF (severity by allow/deny) + structured JSON from the prev-hash trace; redaction preserved (payloads hash-only, metadata exported as KEY NAMES only). Also deduped a latent union-merge duplicate `prev_event_hash` field in recorder.py. Test `test_mobile_siem_export.py` (6) incl. redaction + CLI.
- **T10 ✅ (Phase 12 slice 1)** Org/tenant policy — `mobile/policy_context.py`: signed `OrgPolicyBundle` (HMAC), `OrgPolicyContext`, `TenantPolicyHook` (EnterprisePolicyHook) with deterministic RBAC + ABAC + tenant denials, fail-closed on bad signature; integrates with `explain_capability_policy`. Test `test_mobile_policy_context.py` (7). **Batch 5 (T1-T10) COMPLETE.** Phase 12 slice recorded as main Phase 190. Continuing full roadmap: Phase 11 gate, Phase 12 remainder, Phase 20.
- **Phase 12b ✅ + Phase 11 ✅** feature_flags.py (default-off + kill switch) + capability_gate.py (CapabilityEntryGate: default-denied; requires flag+signed-plan+approval+compliance; ALWAYS routes to fixtures, executed_real_device=False — real device access out of scope/human-gated). Tests: feature_flags(4)+capability_gate(6). Recorded main Phase 191. Remaining roadmap: Phase 12c audit retention, 12d compliance report, 12e SBOM, Phase 20 MCP dev-bridge.
- **Phase 12c/12d/12e ✅ + Phase 20 ✅** audit_retention.py (TTL/rotation), compliance/report.py (aggregated advisory report + CLI), sbom.py (CycloneDX + CLI), mcp_bridge.py (default-off loopback+token+TTL fail-closed guard). Recorded main Phase 192. **Full mobile roadmap (Phases 0–12 + 20) now implemented in simulator-preview posture; Phase 11 is an entry-gate that always routes to fixtures (no real device access).**
