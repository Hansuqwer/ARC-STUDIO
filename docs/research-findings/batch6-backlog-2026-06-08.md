...
- **A4 ✅ (CR-043)** `McpCallDecisionEvent` v2 was defined + in the typed union but never constructed. Added `to_call_decision_event` (McpCallDecision→typed event) + `persist_decision_event` + `next_decision_sequence` in mcp/sandbox.py; wired into server.py so every per-call risk decision now also emits the typed MCP_CALL_DECISION event to `.arc/mcp/decision-events.jsonl`. Test `test_mcp_call_decision_event.py` (5). 128 MCP tests pass.
- **A5 ✅ (CR-045)** New `.github/workflows/dod-gate.yml`: enforces `check-banned-claims.sh` on canonical docs (AGENTS/README/roadmap/phases/checklist) + asserts single-source docs present. YAML valid; gate green locally. **Track A (A1-A5) complete.** Recorded main Phase 193.
- **B1 ✅ + B2 ✅** New CI gates `.github/workflows/mobile-rn.yml` (runs `test_mobile_rn.py` — RN forbidden-symbol + TurboModule contract) and `mobile-flutter.yml` (runs `test_mobile_flutter.py` authoritative + best-effort `flutter analyze`/`test`). Mirror the existing `mobile-expo.yml` gate.
- **B3 ✅ + B4 ✅** Added three mobile gates to `scripts/release_check.sh`: `mobile:native-safety` (recursive forbidden-symbol pytest across expo/rn/flutter), `mobile:sbom` (arc mobile sbom emits CycloneDX), `mobile:compliance` (arc mobile generate compliance-report → requires_human_review). Guarded by uv + runtimes/mobile presence. All 3 gate commands verified locally (30 native tests pass; SBOM+compliance PASS). **Track B complete.**
- **C1-C6 ✅ Track C complete** arc mobile gate/flags/egress/queue/secure-store/audit-retention. 31 CLI tests. Recorded Phase 194.
- **D1-D5 ✅ Track D complete** simulate-through-gate / TenantPolicyHook in policy explain / mypy gate (9 modules) / REAL_VS_MOCK refresh / property+fuzz tests. 115 mobile tests, mypy clean, banned-claims green. Recorded Phase 195. **BATCH 6 COMPLETE (20/20).**
