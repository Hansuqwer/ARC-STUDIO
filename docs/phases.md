...
**Status:** Slices 110.1 + 110.2 + 110.3 Baseline Complete / 110.4–110.6 follow-up | Evidence: cross-repo analysis 2026-06-05; ARC Studio contracts verified. **2026-06-06**: `ArcRuntimeSDKAdapter` (Slice 110.1+110.2) + `mobile_sdk_mapping` (Slice 110.3 — bidirectional `MobileCapability` ↔ SDK CapabilityCard with 25 round-trip tests, all categories + sensitivities covered, SDK-only fields preserved in `metadata`); `arc runtimes --json` discovers the adapter; 42 tests total pass. Truth: simulator/mock only, no run path yet, no native-execution/app-store claims. Companion roadmap item: R79.
