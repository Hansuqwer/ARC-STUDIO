...
**Status:** Slice 110.1 + 110.2 Baseline Complete / 110.3–110.6 follow-up | Evidence: cross-repo analysis 2026-06-05 (`runtimes/Arc-Studio-Mobile-SDK/arc-runtime-sdk/docs/ARC_STUDIO_INTEGRATION_ANALYSIS.md`); ARC Studio contracts verified — `adapters/base.py` (RuntimeAdapter.detect/capabilities/CapabilityReport), `protocol/capabilities.py` (RuntimeCapabilities), `mobile/models.py` (MobileCapability/MobileRuntimeManifest), `runtime_packs/models.py` (RuntimePackManifest). **2026-06-06**: in-repo `adapters/arc_runtime_sdk.ArcRuntimeSDKAdapter` subclasses RuntimeAdapter (detect→tuple with evidence weights; capabilities→RuntimeCapabilities with `can_run=False`; capability_report→CapabilityReport `fake_offline`), registered in `adapters/registry.py`, and `arc runtimes --json` discovers it in a workspace with `arc-sdk.json`; 17 adapter tests pass. Truth: simulator/mock only, no run path yet (no false positive), no native-execution/app-store claims. Companion roadmap item: R79.
