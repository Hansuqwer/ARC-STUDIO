...
| **R-UX2 UX Modes + Approvals** | **Partial** | Status: Partial — widgets built, wiring in progress | Evidence: approval_card.py (140 LOC), capability_banner.py (91 LOC, consumes CAPABILITY_CARD_DECISION), activity_tray.py (137 LOC, consumes MCP_CALL_DECISION + push_mcp_decision), mcp_banner.py (60 LOC) all present. R-UX2 gaps filled this sprint: sandbox-aware shell via decide() at screen.py (commit 12c5afb, 4 tests); PlanView R-012 (plan_view.py 84 LOC, 5 tests). 5140 Python passed. | Notes: Remaining: ApprovalCard event-driven mount into live transcript (widget exists, not yet subscribed in screen.py); streaming already uses is_streaming flag (not static string). CapabilityBanner + ActivityTray widgets complete and consume their typed events. |
