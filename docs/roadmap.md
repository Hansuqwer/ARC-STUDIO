...
| **R-OPEN-HARDEN Production Hardening** | **Partial** | Status: Partial (Phases 123-124: non-streaming + streaming retry shipped) | Evidence: _call_with_retry (Phase 123, both complete() sites) + _stream_with_retry (Phase 124, retry only before first chunk — correctness boundary: post-chunk retry would duplicate non-idempotent LLM output); AuthError/ValidationError propagate immediately; 13 retry tests; 5534 passed. Prior slices: test_harden_retry.py, test_harden_jsonl.py, budget/test_harden_budget.py. | Notes: streaming retry research-grounded (docs/research/roadmap-status-analysis.md §4). Remaining scope: cascading multi-provider failure tests, durability-under-error budget tests. |
