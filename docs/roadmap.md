...
| **R-OPEN-HARDEN Production Hardening** | **Partial** | Status: Partial (Phases 123-126: retry + degrade + multi-provider failover) | Evidence: _call_with_retry (123) + _stream_with_retry (124) + run_turn turn.failed degradation (125) + FallbackProviderClient (126, opt-in ordered failover chain, fails over on retryable errors only, streaming failover pre-first-chunk only); 25 retry/degradation/failover tests; 5546 passed. | Notes: FallbackProviderClient is additive/opt-in (not wired into default path). Failover research-grounded (LiteLLM/Portkey ordered fallback chains). Remaining scope: durability-under-error budget tests (not a clear gap — enforcer not in turn path, persistence 8/8 tested). |
