...
**Created:** 2026-05-17
**Last reality refresh:** 2026-05-31 — R75 macOS direct VZ gated public CLI proof passed once for guest-available `pwd` with no-network/workspace/teardown/audit evidence; default-off and not production-grade; R76 Linux/Firecracker proof remains host-unproven and Linux/KVM-only; R77 remains narrow live-smoke/gated, not broad provider-backed SwarmGraph E2E.
...
| **R-UX1 UX Polish — Header + ContextMeter + Markdown + Token-Driven TCSS** | **Planning Only** | Status: Planning Only | Evidence: UX_AUDIT.md §19 P0 (3-4 day estimate); audit scores TUI at 39/100 on 25 heuristics; R-001 (Header) + R-002 (ContextMeter) are the remaining gaps. Verification 2026-06-05: R-003 (ModeBadge/Shift+Tab) at screen.py:47-48 SHIPPED; R-004 partial (MarkdownBlock 36 lines at transcript.py:43-49) SHIPPED; ContextMeter absent from status_bar.py — audit correct there. | Notes: R-UX1 scope narrows to R-001 + R-002 only (~2 days, not 3-4) given partial v0.2.0-alpha work. Audit §23 recommendation: highest-leverage single PR available. No backend changes needed. Targets TUI grade 39 → mid-60s. |
| **R-UX2 UX Modes + Approvals** | **Planning Only** | Status: Planning Only | Evidence: UX_AUDIT.md §19 P1 (1 week estimate); R-006 (CapabilityCardBanner), R-007 (ApprovalCard), R-008 (ActivityTray/MCP banner), R-012 (PlanView), streaming improvements, sandbox-aware shell. Depends on R-UX1 token-driven TCSS for theming. | Notes: First UX phase requiring backend wiring (capability cards subscription, MCP decisions stream, sandbox routing). Closes 4 of 6 P0 first-run pain moments from audit §5. |
| **R-UX3 UX Components + Information Architecture** | **Planning Only** | Status: Planning Only | Evidence: UX_AUDIT.md §19 P2 (2 week estimate); R-005 (ToolCard rebuild), R-009 (DiffViewer), R-010 (SlashMenu), R-011 (CommandPalette), R-014-016 (Toaster/KeycapHint/RiskBadge), R-017-020 (views polish), frontmatter-driven slash registry. | Notes: Largest phase. Includes slash-command frontmatter migration that makes v0.5.1+ slash work (/wallet, /budget, /expand, /models, /model-info) consistently documented. |
| **R-UX4 UX Themes + Accessibility** | **Planning Only** | Status: Planning Only | Evidence: UX_AUDIT.md §19 P3 (1 week estimate); R-024 (5 themes: TokyoNight, Catppuccin Mocha, Catppuccin Latte, High-Contrast, Monochrome); NO_COLOR glyph fallback; reduced motion; pytest-textual-snapshot harness; /title /statusline commands. | Notes: Closes audit heuristics H11, H12, H22, H25. pytest-textual-snapshot harness gates future TUI changes on diff. ux/mockups/ not preserved (generated in audit workspace, not copied to repo). |
| **R-OPEN-HARDEN Production Hardening** | **Research Intake** | Status: Research Intake | Evidence: referenced in 2026-06-05 handover doc as candidate "Option A (hardening, momentum)"; no spec written; no scope defined. | Notes: Needs sprint planning. Possible scope: failure-injection tests, retry/backoff across providers, durability tests beyond v0.4.1 SQLite persistence. |
| **R-OPEN-SANDBOX MicroVM / Sandbox Layer** | **Research Intake** | Status: Research Intake | Evidence: referenced in AGENTS.md as "original mandate"; in handover as "Option B"; no spec written; audit §5 notes shell escape in tui/screen.py:271-291 bypasses security/sandbox.py via raw subprocess.run(shell=True). | Notes: Cross-cuts R-UX2 ("sandbox-aware shell"). Needs scoping conversation before becoming a sprint. |
| **R-OPEN-DEFERRED-RUNBOOKS Execute Deferred Research Runbooks** | **Research Intake** | Status: Research Intake | Evidence: token-estimator-accuracy.md and budget-persistence-audit.md exist as runbooks; never executed against live repo. | Notes: Half-day each. Outcomes inform R-01 wallet display semantics (≈ qualifier) and v0.4.1 persistence design honesty. |
