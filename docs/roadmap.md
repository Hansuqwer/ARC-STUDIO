...
| **R-TS3 arc-protocol-ts coverage backfill** | **Baseline Complete** | Status: Not Started | Evidence: typed event additions (run-events.ts) halved effective TS coverage; arc-protocol-ts branches 56% → 37%, functions 94% → 44%, lines 79% → 46%, statements 75% → 45%; thresholds reset to unblock CI in `898ee26` | Notes: Backfill tests for new typed events (CAPABILITY_CARD_DECISION, MCP_CALL_DECISION, EVAL_POLICY_*, BATCH_VOTE_*, etc.) to restore ~75/78/90/55 thresholds. Tracked as follow-up to v0.3.0-alpha. |
