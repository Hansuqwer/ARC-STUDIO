...
| R-AUDIT26 | IDE McpWorkbench risk-badge color + aria-label (a11y) | Not Started | Evidence: — | Notes: `McpWorkbenchTab.tsx` `RISK_VARIANT` maps both `high` and `critical` → `'info'` (identical color, WCAG 1.4.1 use-of-color); risk badge has no `aria-label`. TUI RiskBadge shipped (R-UX3); IDE side open. Source: `docs/research-findings/unified-implementation-backlog-2026-06-07.md` SLICE 10 (P0). |
