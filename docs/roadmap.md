...
**Status:** Partial — P0 adapter + registry + schema mapping shipped (Slices 110.1–110.3); 110.4–110.6 follow-up | Evidence: 2026-06-06 `ArcRuntimeSDKAdapter` (can_run=False fake mode, registered, discovered by `arc runtimes`) + `mobile_sdk_mapping` (bidirectional `MobileCapability` ↔ SDK CapabilityCard, all categories/sensitivities, SDK-only fields in metadata, 25 round-trip tests); 42 adapter+mapping tests pass | Notes: runtime-pack/daemon parity (110.4/110.5), optional Theia/TUI surfacing (110.6) remain. Simulator/mock only; no native-execution or app-store claims. Execution plan: `docs/phases.md` Phase 111.
