...
| **R-OPEN-HARDEN Production Hardening** | **Partial** | Status: Partial (Phases 123-125: non-streaming + streaming retry + graceful turn-level degradation) | Evidence: _call_with_retry (123) + _stream_with_retry (124, retry only pre-first-chunk) + run_turn ProviderError except block (125: emits turn.failed + degraded TurnResult instead of crashing, mirrors Cancelled path); 16 retry/degradation tests; 5537 passed. Prior slices: test_harden_retry.py, test_harden_jsonl.py, budget/test_harden_budget.py. | Notes: streaming retry research-grounded (docs/research/roadmap-status-analysis.md §4). Remaining scope: cascading multi-provider failover (would need a provider-router abstraction — larger), durability-under-error budget tests. |
