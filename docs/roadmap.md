...
| **R-UX3 UX Components + Information Architecture** | **Partial** | Status: Partial — several components present, wiring + missing pieces remain | Evidence: slash_menu.py wired in screen.py; command_palette.py (Ctrl+P) wired; tool_card.py present (not wired in transcript); toaster.py present (not wired in screen); diff_block.py present (no DiffViewer view). Missing: KeycapHint, RiskBadge, frontmatter slash registry, views polish (ToolCard/Toaster wiring, /wallet /budget /models consistency). 2 week estimate remaining. |
