...
**Current:** Baseline Complete. `StreamingAuditVerifier` class (hmac, sha256, auto modes), `arc audit verify` CLI with memory-bounded streaming (configurable 1-500 MB), full HMAC key lifecycle (`AuditKeyManager`), fail-closed HMAC appends when no audit key is available, signed `seq`/`timestamp`/`key_id` metadata for new records, legacy verification compatibility for already-written HMAC chains, and mixed payload-shape support inside signed chain records. Daemon `session_changed` events are explicitly classified as ephemeral and excluded from per-run audit-chain coverage unless a future run path persists them inside chain records.
