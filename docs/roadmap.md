...
**Status:** Partial — Slices 110.1–110.4 shipped; 110.5–110.6 follow-up | Evidence: 2026-06-06 `ArcRuntimeSDKAdapter` (can_run=False fake mode, registered), `mobile_sdk_mapping` (bidirectional MobileCapability↔SDK card, 25 tests), `arc_runtime_sdk_pack` (arc-sdk.json→RuntimePackManifest converter, auto-synthesises permissions for R7, example SDK project validates ok=True, 13 tests); total 55 adapter/pack/mapping tests pass | Notes: daemon/protocol parity (110.5), optional Theia/TUI surfacing (110.6) remain. Simulator/mock only; no native-execution or app-store claims. Execution plan: `docs/phases.md` Phase 111.
