...
**Created:** 2026-05-17
**Last reality refresh:** 2026-05-31 — R75 macOS direct VZ proof passed once and local artifact provenance tooling exists, but public macOS microVM execution remains blocked; R76 Linux/Firecracker proof remains host-unproven and Linux/KVM-only; R77 remains narrow live-smoke/gated, not broad provider-backed SwarmGraph E2E.
...
| **R75 macOS MicroVM Execution + Strict No-Network Proof** | **Host Proof Passed / Artifact Provenance Added / Public Exec Blocked** | **Phase 104 — Direct Apple VZ no-NIC proof passed once and `arc sandbox vz-artifacts` can hash-pin local proof artifacts; public macOS microVM execution remains blocked** |
...
**Current:** Direct Apple Virtualization.framework no-NIC host proof passed once, not via Lima. ARC has `VZNoNetworkProof` preflight/opt-in runner scaffolding, `arc sandbox doctor --json` reports `vz_no_nic` with `networkDevices=[]`, and `tools/arc-vz-runner.swift` configures `networkDevices=[]`. The gated run used `ARC_VZ_PROOF=1`, a compiled/signed runner, ARM64 kernel/initrd, and guest markers proving boot, no guest ethernet, no default route, network probe failure, workspace sentinel read, symlink escape blocked, command result markers, and teardown ok. `arc sandbox vz-artifacts` now creates a local proof-only artifact set with runner source, entitlements, optional compiled/signed runner, copied kernel/initrd, SHA256 manifest, `networkDevices=[]`, and `public_execution_enabled=false`; it does not download assets or boot a VM. Lima remains a low-security harness only because its default/user-mode networking is network-present. Public macOS `arc sandbox run --provider microvm` execution remains blocked.

**Status:** Host Proof Passed / Artifact Provenance Added / Public Execution Blocked | Evidence: `cd python && ARC_VZ_PROOF=1 ARC_VZ_RUNNER=/var/folders/dp/1fh07k_922j5qk7xfncn1zv40000gn/T/opencode/arc-vz-runner ARC_VZ_KERNEL=/var/folders/dp/1fh07k_922j5qk7xfncn1zv40000gn/T/opencode/arc-vz-proof/debian-linux ARC_VZ_INITRD=/var/folders/dp/1fh07k_922j5qk7xfncn1zv40000gn/T/opencode/arc-vz-proof/arc-vz-proof-initrd.gz ARC_VZ_TIMEOUT_SECONDS=45 uv run pytest tests/isolation/test_vz_proof.py -v` → 9 passed; `cd python && uv run arc sandbox vz-artifacts --json --output /var/folders/dp/1fh07k_922j5qk7xfncn1zv40000gn/T/opencode/arc-vz-artifacts --kernel /var/folders/dp/1fh07k_922j5qk7xfncn1zv40000gn/T/opencode/arc-vz-proof/debian-linux --initrd /var/folders/dp/1fh07k_922j5qk7xfncn1zv40000gn/T/opencode/arc-vz-proof/arc-vz-proof-initrd.gz --build-runner` → blockers `[]` | Notes: This proves the gated VZ proof harness and local artifact generation on this host only. It does not enable or claim production-grade/public macOS microVM execution; remaining work is public provider wiring, audit/output caps, timeout/SIGINT cleanup, host CI, broader failure-mode tests, and upstream kernel/initrd provenance/distribution policy.
