...
| **R-OPEN-HARDEN Production Hardening** | **Partial** | Status: Partial (Phase 123 shipped provider-call retry hardening) | Evidence: _call_with_retry in runtime/turn_manager.py wires existing RateLimitError/NetworkError retry infrastructure into both complete() call sites; AuthError/ValidationError propagate immediately; 8 new tests; 5529 passed. Prior slices: test_harden_retry.py (retry data structures), test_harden_jsonl.py (poison-pill JSONL), budget/test_harden_budget.py (concurrent budget). | Notes: Streaming path retry out of scope for this slice. Task-level retry (tasks/models.py) already exists. Remaining scope: cascading provider failure tests, durability-under-error budget tests. |
