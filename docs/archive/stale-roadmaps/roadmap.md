...
| R56 Token-Saving Research Intake | Research Intake (Planning Only) | `docs/research/TOKEN_SAVING_PLAN.md` on branch `research/token-saving-plan`; HEAD `d82e925`; 12 recommendations (R-01..R-12); top-3 quick wins = OTel cache fields + `/compact` deterministic + Anthropic cache_control populator; full source map of `budget/schema.py`, `providers/anthropic.py:251-401`, `providers/budget_preflight.py`, `protocol/cost_record.py`, `observability/otel_mapping.py`, `simulation/simulator.py`, `capabilities/CostCapability` | Repo-grounded plan for v0.3.0-alpha token-saving sprint. Confirms ARC has more cost infrastructure than competitors (scope-based BudgetEnforcer, typed CostRecord v3, cache_control plumbing complete in `providers/anthropic.py:251-366`, SimulatedCost, FlightRecorder hash chain) but key call sites unsubscribed. Recommendation: ship R-03 (OTel cache fields, ~30 LOC) first to unlock measurement of R-04 (Anthropic cache populator, ~80 LOC). Does NOT change any Baseline Complete status. Extended 12-recommendation artifact: `docs/research/TOKEN_SAVING_PLAN_EXTENDED.md`. |
