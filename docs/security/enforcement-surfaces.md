...
- `arc sandbox vz-artifacts --exec-init --output <dir> --json` writes a reviewable VZ guest init contract plus manifest. `--pack-initrd --busybox <path>` can package a gzip `newc` initramfs using static local BusyBox and local `cpio`; it rejects dynamically linked BusyBox for the minimal standalone initramfs path and does not download assets, bundle Python, or boot a VM.
...
## Phase 159 — Inventory/Context + Storage + Provider-Error Hardening (R-POLISH1)

| ID | Surface | Change | Where | Test |
|----|---------|--------|-------|------|
| P159-1 | Workspace inventory / context file enumeration | Sensitive files (`.env*`, `credentials.json`, `secrets.yaml`, `*.pem`/`*.key`/`*.p12`/`*.pfx`, `id_rsa`, `.npmrc`, `.netrc`, `.pgpass`, `.git-credentials`, ...) are never enumerated or read into context packs, regardless of the requested suffix filter. Complements sandbox-layer PR-B (which gates command-time secret reads). | `workspace.py:is_sensitive_file`/`iter_workspace_files`, `context/providers/local_repo.py` | `tests/test_workspace.py` |
| P159-2 | Provider error surfacing | Provider SDK exception text is run through the canonical `redact_secrets()` before wrapping, so API keys / bearer tokens / response bodies never reach logs/UI/audit. Error-type classification is preserved. | `providers/openai_compatible.py:_map_error`, `providers/anthropic.py:_map_error`, `security/redaction.py` | `tests/test_provider_error_redaction.py` |
| P159-3 | Trace/artifact/receipt path construction | Fail-closed run-ID guard rejects path separators and `..` before building JSONL paths (defense-in-depth behind the MCP-layer `INVALID_MCP_ARGUMENT` check). | `storage/jsonl.py:_safe_run_id` | `tests/test_storage.py::TestJsonlRunIdGuard` |
